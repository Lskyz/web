...
struct CustomWebView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        if webView == nil {
            ...
            controller.add(context.coordinator, name: "debugLog")
    ...
    class Coordinator: NSObject, WKUIDelegate, UIScrollViewDelegate, WKScriptMessageHandler, UIGestureRecognizerDelegate {
        ...
        func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
            if message.name == "debugLog" {
                let prefix = "🛰️ "
                let clamp: (String) -> String = { text in
                    guard text.count > 600 else { return text }
                    let idx = text.index(text.startIndex, offsetBy: 600)
                    return String(text[..<idx]) + "…"
                }
                if let text = message.body as? String {
                    TabPersistenceManager.log(prefix + clamp(text))
                } else if let dict = message.body as? [String: Any] {
                    let pairs = dict.map { "\($0.key)=\(String(describing: $0.value))" }.sorted().joined(separator: ", ")
                    TabPersistenceManager.log(prefix + clamp(pairs))
                } else if let array = message.body as? [Any] {
                    let joined = array.map { String(describing: $0) }.joined(separator: ", ")
                    TabPersistenceManager.log(prefix + clamp(joined))
                } else {
                    TabPersistenceManager.log(prefix + clamp(String(describing: message.body)))
                }
                return
            }
