...
struct BFCacheSnapshot: Codable {
    ...
    func restore(to webView: WKWebView, completion: @escaping (Bool) -> Void) {
        ...
        // 🔥 수정: async IIFE로 완전히 감싸서 Promise 반환 보장
    ...
    // MARK: - 🔥 **수정: async IIFE로 완전 래핑하여 Promise 반환 보장**
    
    private func generateAsyncRestorationScript(anchors: UnifiedAnchors) -> String {
        return """
        // 🔥 async IIFE로 완전히 감싸서 Promise 반환 보장
        (async function() {
            const logs = [];
            const startTime = Date.now();
            
            try {
                logs.push('🎯 통합 앵커 복원 시작 (async context)');
                logs.push('파라미터 확인:');
