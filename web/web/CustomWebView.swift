...
//  - ✨ 스와이프 제스처 즉시 UI 동기화 - 버튼 활성화 지연 문제 해결
//
...
class Coordinator: NSObject, WKUIDelegate, WKNavigationDelegate, UIScrollViewDelegate, WKScriptMessageHandler {
    ...
    // MARK: - ✨ 즉시 네비게이션 상태 동기화 헬퍼
    private func syncNavigationStateImmediately(_ webView: WKWebView, context: String) {
        DispatchQueue.main.async {
            let canGoBack = webView.canGoBack
            let canGoForward = webView.canGoForward
            
            // 상태가 다르면 즉시 업데이트
            if self.parent.stateModel.canGoBack != canGoBack {
                self.parent.stateModel.canGoBack = canGoBack
            }
            if self.parent.stateModel.canGoForward != canGoForward {
                self.parent.stateModel.canGoForward = canGoForward
            }
            
            TabPersistenceManager.debugMessages.append("🔄 [\(context)] 즉시 동기화: Back=\(canGoBack), Forward=\(canGoForward)")
        }
    }

    // MARK: - ✨ WKNavigationDelegate (에러 처리 강화 + 즉시 UI 동기화)
    ...
    func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
        ...
        // ✨ 네비게이션 버튼 상태 즉시 동기화
        syncNavigationStateImmediately(webView, context: "didStart")
    ...
    func webView(_ webView: WKWebView, didCommit navigation: WKNavigation!) {
        // ✨ 커밋 시점에서도 즉시 동기화 (확실하게)
        syncNavigationStateImmediately(webView, context: "didCommit")
        
        // 기존 StateModel의 didCommit 호출 (쿠키 동기화 등)
        parent.stateModel.webView(webView, didCommit: navigation)
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        // ✨ 최종 네비게이션 상태 동기화
        syncNavigationStateImmediately(webView, context: "didFinish")
    ...
    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        ...
        // ✨ 실패해도 네비게이션 상태는 동기화
        syncNavigationStateImmediately(webView, context: "didFailProvisional")
    ...
    func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        ...
        // ✨ 실패해도 네비게이션 상태는 동기화
        syncNavigationStateImmediately(webView, context: "didFail")
